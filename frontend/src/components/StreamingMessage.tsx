...
// Per-model pill colors — consistent across streaming + completed messages so
// the user can visually confirm which model ran each reply.
const MODEL_PILL_STYLE: Record<ModelChoice, React.CSSProperties> = {
        ...
        {!isUser && model && (
          <div className="mb-1.5">
            <span
              ...
              style={MODEL_PILL_STYLE[model]}
              title={`This response is being generated by ${model}`}
            >
              <span
                aria-hidden
                className="inline-block w-1.5 h-1.5 rounded-full"
                style={{ background: MODEL_PILL_STYLE[model].color }}
              />
              {model}
            </span>
          </div>
